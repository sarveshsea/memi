    ...
    <section className="chat-quality-layer" data-chat-qol="codex-antigravity" aria-label="Chat quality controls">
      ...
      <div className="chat-qol-grid">
        ...
        <section className="chat-memory-pins" data-memory-pins="session">
          ...
          {props.memoryPins.slice(0, 2).map((pin) => <span key={pin} title={pin}>{trimText(pin, 36)}</span>)}
        </section>
  ...
  return (
    <>
      <WorkbenchPanel className="context-search-panel" eyebrow="Memory" title="Memory / Context" meta={String(props.contextItems.length)}>
        ...
        <MemoryTable compact title="Memory / Context" items={props.contextItems} empty="No context indexed." onOpen={props.onOpenItem} />
        {detailItem ? (
      ...
      >
        <div className="agent-task-list" data-agent-tasks="trace-tasks">
          {props.traceModel.tasks.length === 0 ? <p className="empty">No active trace.</p> : null}
  ...
  return (
    <WorkbenchPanel className="agent-logs-panel" eyebrow="Logs" title="Agent Logs" meta={session?.harness ?? "idle"}>
      <div className="agent-log-list" data-agent-logs="raw-events">
        {visibleEvents.length === 0 ? <p className="empty">No events.</p> : null}
  ...
  return (
    <aside className="project-sidebar" data-project-sidebar="codex-style" data-sidebar-collapsed={String(props.collapsed)} aria-label="Projects">
      <div className="project-sidebar-main">
        ...
        <div className="project-sidebar-folders" data-project-folder-list="true">
          ...
          {projects.length === 0 ? <p className="empty">No sessions.</p> : null}
        </div>
      </div>
...
export function StudioControlIcon({ name }: { name: "attach" | "mode" | "access" | "plan" | "harness" | "action" | "command" | "details" | "light" | "dark" | "settings" | "refresh" | "search" | "pin" | "branch" | "copy" | "context" | "collapse" | "expand" }) {
...
export function ChangedFilesPanel(props: {
  ...
  return (
    <section className="inline-changed-files" data-changed-files-panel="inline-review">
      <div className="drawer-section-head">
        <span>{clean ? "Clean" : props.trace?.reviewLabel ?? "Changed"}</span>
      ...
      {files.length > 0 ? (
        <details className="changed-file-disclosure" open={showFiles} onToggle={(event) => setShowFiles(event.currentTarget.open)}>
          <summary>{`Show ${previewFiles.length}`}</summary>
          {showFiles ? (
  ...
  return (
    ...
    >
      ...
      <div className="inline-actions">
        <button data-action-id="figma.connect" type="button" onClick={props.onConnect} disabled={bridgeButtonDisabled}>
          {bridgeButtonLabel}
        </button>
        <button data-action-id="figma.disconnect" type="button" onClick={props.onDisconnect} disabled={!props.figmaStatus?.running || props.figmaActionRunning}>Stop</button>
        <button data-action-id="figma.open" type="button" onClick={props.onOpen} disabled={!isBridgeRunning || props.figmaActionRunning}>Open</button>
        <button className="primary" data-action-id="settings.save" type="button" onClick={props.onSaveSettings} disabled={!props.settingsDraft}>Save</button>
      </div>
                ...
                <div className="review-section-body">
                  <div className="review-toolbar">
                    <button
                      ...
                      type="button"
                      onClick={() => props.onReviewSection(artifact.id, section.id, "looks_good", `${section.title} verified in Studio.`)}
                    >
                      OK
                    </button>
                    <button
                      ...
                      type="button"
                      onClick={() => props.onReviewSection(artifact.id, section.id, "needs_work", `${section.title} needs a follow-up pass.`)}
                    >
                      Fix
                    </button>
                  </div>
...
function DesignSystemSandbox({ artifact, figmaStatus }: { artifact: DesignSystemArtifact; figmaStatus: FigmaStatus | null }) {
  ...
  return (
    <section className="design-system-sandbox" data-design-system-sandbox="local-container">
      <article><strong>Reference container</strong><span>{artifact.sections.length} sections / {artifact.sourceRefs.length} sources</span></article>
      <article><strong>Agent playground</strong><span>{artifact.createdByHarness} / JSON artifact</span></article>
      <article><strong>Figma compatibility</strong><span>{bridge}{figmaStatus?.port ? ` / :${figmaStatus.port}` : ""}</span></article>
      <article><strong>Developer handoff</strong><span>tokens / components / source refs</span></article>
    </section>
  );
}
...
export function MermaidBoardSurface(props: {
  ...
  return (
    <section className="mermaid-board-surface" data-mermaid-board="studio-sandbox" aria-label="Mermaid Board sandbox">
      <header className="mermaid-board-header">
        <div>
          <p className="eyebrow">Mermaid Board / FigJam-style sandbox</p>
          <h2>{board?.title ?? "Studio Mermaid Board"}</h2>
          <span>{board ? `${board.nodes.length} nodes / ${board.edges.length} connectors` : "Create a local board"}</span>
        </div>
        <div className="mermaid-board-actions">
          <button data-action-id="board.create" type="button" onClick={() => void props.onCreate()} disabled={props.loading}>Create</button>
          <button data-action-id="board.add_node.mermaid" type="button" onClick={() => void props.onAddNode("mermaid")} disabled={props.loading}>Mermaid</button>
          <button data-action-id="board.add_node.sticky" type="button" onClick={() => void props.onAddNode("sticky")} disabled={props.loading}>Sticky</button>
          <button data-action-id="board.layout" type="button" onClick={() => void props.onLayout()} disabled={props.loading || !board}>Layout</button>
          <button data-action-id="board.export_mermaid_jam" type="button" onClick={() => void props.onExport()} disabled={props.loading || !board}>Export</button>
        </div>
      </header>
      {props.error ? <p className="mermaid-board-error">{props.error}</p> : null}
      <div className="mermaid-board-layout">
        <div className="mermaid-board-canvas" data-mermaid-board-canvas="tldraw-mermaid">
          <div className="mermaid-board-tldraw" aria-hidden="true">
            <Tldraw persistenceKey={`memoire-${board?.id ?? "mermaid-board"}`} />
          </div>
          <div className="mermaid-board-overlay">
            {board ? (
              <svg className="mermaid-board-edges" aria-hidden="true">
                {board.edges.map((edge) => {
                  const from = board.nodes.find((node) => node.id === edge.fromNodeId);
                  const to = board.nodes.find((node) => node.id === edge.toNodeId);
                  if (!from || !to) return null;
                  const x1 = from.position.x + from.position.width;
                  const y1 = from.position.y + from.position.height / 2;
                  const x2 = to.position.x;
                  const y2 = to.position.y + to.position.height / 2;
                  return (
                    <g key={edge.id}>
            ...
            {board?.frames.map((frame) => (
              <section
                className="mermaid-board-frame"
                data-mermaid-board-frame={frame.id}
                key={frame.id}
                style={{ left: frame.position.x, top: frame.position.y, width: frame.position.width, height: frame.position.height }}
              >
            ...
            {board?.nodes.map((node) => (
              <button
                className={`mermaid-board-node ${selectedNode?.id === node.id ? "active" : ""}`}
                data-mermaid-board-node={node.kind}
                data-node-id={node.id}
                key={node.id}
                style={{ left: node.position.x, top: node.position.y, width: node.position.width, minHeight: node.position.height }}
                type="button"
                onClick={() => setSelectedNodeId(node.id)}
              >
                <span>{node.kind}</span>
                <strong>{node.title}</strong>
                <small>{trimText(node.body || node.mermaidSource || "No body", 120)}</small>
                {node.researchBacking.length ? <em>{node.researchBacking.length} evidence refs</em> : null}
              </button>
            ))}
            {!board ? (
              <div className="mermaid-board-empty">
                <strong>Board not loaded</strong>
                <span>Create a local Mermaid Board to let agents and humans sketch product flows.</span>
              </div>
            ) : null}
          </div>
        </div>
        <aside className="mermaid-board-inspector" data-mermaid-board-inspector="node-detail">
          {selectedNode ? (
            <>
              <span>{selectedNode.kind} / {selectedNode.author}</span>
              <h3>{selectedNode.title}</h3>
              <p>{selectedNode.body || "No description yet."}</p>
              {selectedNode.mermaidSource ? <MermaidPreview source={selectedNode.mermaidSource} /> : null}
              <div className="mermaid-board-source-list">
                {selectedNode.researchBacking.map((source) => <small key={source}>{source}</small>)}
                {selectedNode.sourceEventIds.map((eventId) => <small key={eventId}>{eventId}</small>)}
              </div>
            </>
          ) : (
            <p>Select a node to inspect Mermaid source, evidence links, and agent authorship.</p>
          )}
          <section className="mermaid-board-export-list" data-mermaid-board-export="source-open">
            <strong>{props.exports.length ? "Latest export" : "Export boundary"}</strong>
            <span>Source + open. No clipboard or direct FigJam paste automation.</span>
...
function MermaidPreview({ source }: { source: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    const id = `mermaid-${actionIdSegment(source).slice(0, 32)}-${Date.now().toString(36)}`;
  ...
  if (svg) return <div className="mermaid-board-preview" data-mermaid-preview="rendered" dangerouslySetInnerHTML={{ __html: svg }} />;
  return (
    <pre className="mermaid-board-preview" data-mermaid-preview={error ? "error" : "source"}>
      {error ? `Mermaid render warning: ${error}\n\n${source}` : source}
    </pre>
  );
}
      ...
      <>
        ...
        <div className="permission-grid">
          {Object.entries(permissions).map(([permission, value]) => (
            <button data-action-id={`settings.permission.${permission}`} key={permission} type="button" onClick={() => props.onOpenMacOSPermission(permission)}>
  ...
  return (
    <div className="formatted-message">
      {nodes.map((node, index) => {
        ...
        return node.kind === "li"
          ? <li key={index}>{formatInlineMessage(node.text)}</li>
          : <p key={index}>{formatInlineMessage(node.text)}</p>;
      })}
    </div>
  );
}
...
function formattedNodes(text: string): FormattedNode[] {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    nodes.push({ kind: looksLikeResultHeading(heading) ? "heading" : "p", text: heading });
  }
  flushTable();
  return nodes;
}
...
function permissionLabel(value: string): string {
  if (value === "screenRecording") return "Screen Recording";
  if (value === "fileAccess") return "Files and Folders";
...
function memoryFilterCounts(items: ProjectMemoryItem[]) {
  return [
    ...
    { id: "page", label: "Pages", count: items.filter((item) => /page|home|changelog/i.test(`${item.kind} ${item.tags.join(" ")}`)).length },
    { id: "component", label: "Components", count: items.filter((item) => /component|atom|molecule|organism/i.test(`${item.title} ${item.tags.join(" ")}`)).length },
    { id: "token", label: "Tokens", count: items.filter((item) => /token|color|type|spacing/i.test(`${item.title} ${item.tags.join(" ")}`)).length },
    { id: "reference", label: "References", count: items.filter((item) => /reference|source|refer/i.test(`${item.title} ${item.tags.join(" ")} ${item.sourcePath}`)).length },
    { id: "markdown", label: "Markdown", count: items.filter((item) => /\.mdx?$/i.test(item.sourcePath)).length },
    { id: "yaml", label: "YAML", count: items.filter((item) => /\.ya?ml$/i.test(item.sourcePath)).length },
  ];
}
