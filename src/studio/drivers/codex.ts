/**
 * CodexDriver — first proof-of-pattern driver in the new ProviderRuntime contract.
 *
 * This driver does not yet replace the existing Codex subprocess management in
 * `src/studio/harnesses.ts`. It demonstrates the new contract end-to-end:
 *
 *   1. Subprocess transport is injected (`spawnTransport`) so tests run without
 *      a real `codex` binary on PATH.
 *   2. Lifecycle drives through the SessionMachine.
 *   3. Every observable thing emits a canonical `ProviderRuntimeEvent`.
 *   4. Errors are typed `HarnessError`s, not raw Errors.
 *
 * The cut-over to live Codex (replacing the old code path) lands behind the
 * `STUDIO_USE_NEW_HARNESS_LAYER=1` feature flag once all 7 drivers exist.
 */
...
export interface CodexTransport {
  /** Send a line of input to the underlying codex process. */
  write(line: string): Promise<void>;
  /** Cleanly close the process. */
  close(reason?: string): Promise<void>;
  /** Force-terminate the process. */
  kill(signal?: NodeJS.Signals): Promise<void>;
  /** Subscribe to stdout/stderr lines. Returns an unsubscribe. */
...
export interface CodexTransportFactory {
  (opts: { model: string; effort: string; env: NodeJS.ProcessEnv; cwd: string }): Promise<CodexTransport>;
}

export interface CodexDriverOptions extends Record<string, unknown> {
  model?: string;
  effort?: "low" | "medium" | "high" | "xhigh";
  cwd?: string;
  env?: NodeJS.ProcessEnv;
  spawnTransport: CodexTransportFactory;
}

export class CodexDriver extends BaseHarnessDriver {
  ...
  private opts(): CodexDriverOptions {
    const raw = (this.config.options ?? {}) as CodexDriverOptions;
    if (typeof raw.spawnTransport !== "function") {
      throw new Error("CodexDriver requires options.spawnTransport (a CodexTransportFactory)");
    }
    return raw;
  }
  ...
  start(): Effect.Effect<void, HarnessError> {
    ...
    return Effect.tryPromise({
      try: async () => {
        ...
        self.emit({ ...self.envelope(), type: "session.created", harnessConfigSummary: { harness: self.config.harnessId, model: opts.model, effort: opts.effort } });
  ...
  sendTurn(req: HarnessTurnRequest): Effect.Effect<void, HarnessError> {
    ...
    return Effect.tryPromise({
      try: async () => {
        if (!self.transport) {
          throw new Error("CodexDriver.sendTurn called before start()");
        }
  ...
  private handleLine(line: string, stream: "stdout" | "stderr"): void {
    ...
    try {
      ...
      this.emitDiagnostic(stream === "stderr" ? "error" : "warn", `non-json from codex: ${trimmed.slice(0, 200)}`);
      return;
    }
  ...
  private handleExit(code: number | null, signal: NodeJS.Signals | null): void {
    ...
    if (!ok) {
      const err = harnessSubprocessError({
        ...
        message: `codex subprocess exited with code=${code} signal=${signal ?? "(none)"}`,
        exitCode: code,
        signal,
      });
...
/** Canonical HarnessId for the Codex driver. Exported so callers can ask the
 *  registry for it without re-deriving the prefix convention. */
export const CODEX_HARNESS_ID = asId("HarnessId", "hns_codex");

registerDriver(CODEX_HARNESS_ID, (config) => new CodexDriver(config));
